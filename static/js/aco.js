export function initAnts(canvas, options = {}) {
  ...
  async function initTooltips() {
    const tooltipData = {
      'alpha': {
        ...
        description: 'Степень использования накопленного опыта в правиле выбора $p_{ij}^k(t) \\propto [\\tau_{ij}(t)]^{\\alpha}[\\eta_{ij}]^{\\beta}$. При $\\alpha \\to 0$ игнорируется история, при увеличении – усиливается сходимость к найденным решениям.'
      },
      'beta': {
        title: 'Влияние эвристики $\\beta$',
        description: 'Степень учёта априорной «желательности» $\\eta_{ij}$ в $p_{ij}^k(t)$. При $\\beta \\to 0$ эвристика игнорируется, при увеличении – доминирует жадная стратегия выбора ближайших узлов.'
      },
      'rho': {
        title: 'Коэффициент испарения $\\rho \\in (0,1]$',
        description: 'Мера «забывания» в динамике $\\tau_{ij}(t+1) = (1-\\rho)\\tau_{ij}(t) + \\sum_k \\Delta\\tau_{ij}^k(t)$. Большие $\\rho$ укорачивают память системы и способствуют исследованию пространства решений.'
      },
      'Q': {
        title: 'Интенсивность подкрепления $Q$',
        description: 'Масштаб откладываемого феромона $\\Delta\\tau_{ij}^k(t) = Q/L_k(t)$ на рёбрах решения. Линейно усиливает контраст между хорошими и плохими путями.'
      },
      'm': {
        title: 'Численность колонии $m$',
        description: 'Количество независимых агентов. Увеличение снижает дисперсию оценки и ускоряет обнаружение качественных решений за счёт параллельного исследования.'
      },
      'T': {
        title: 'Бюджет итераций $T$',
        description: 'Число глобальных циклов «решение–обновление». Прямо ограничивает время работы и глубину стагнации в локальных оптимумах.'
      },
      'tau0': {
        title: 'Начальная концентрация феромона $\\tau_0$',
        description: 'Инициализационное значение $\\tau_{ij}(0) = \\tau_0$ на всех рёбрах (дугах). Большие значения $\\tau_0$ делают начальный поиск более случайным.'
      },
      'graphType': {
        title: 'Тип графа (неориентированный/ориентированный)',
        description: 'Определяет симметрию феромонов: для неориентированного случая $\\tau_{ij} = \\tau_{ji}$ и $w_{ij} = w_{ji}$, для ориентированного – независимые значения на противоположных дугах.'
      },
      'startDist': {
        title: 'Распределение стартовых вершин',
        description: 'Закон выбора начальной вершины $i_0$ для каждого муравья: равномерно по $V$ либо по заданному распределению (например, фиксированная стартовая точка).'
      },
      'seed': {
        title: 'Инициализация ГПСЧ',
        description: 'Фиксация состояния ГПСЧ для воспроизводимости траекторий построения решений и последовательности генерации графов.'
      }
    };
  ...
  // Обработчик изменения размера окна - ИСПРАВЛЕН!
  function handleResize() {
    // Для превью используем размеры родительского контейнера
    const newWidth = params.isPreview ? canvas.clientWidth : window.innerWidth;
    const newHeight = params.isPreview ? canvas.clientHeight : window.innerHeight;
  ...
  // Привязываем обработчик изменения размера только для полноэкранного режима
  if (!params.isPreview) {
